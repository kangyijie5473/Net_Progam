...
void Network::setAlarm()
{
    ...
    timefd_settime(_timerFd, 0, ,NULL)
}
int Network::initMainLoop() {
    _timerFd = timefd_create(CLOCK_REALTIME, TFD_NONBLOCK);
...
template<typename T>
int Network::startMainLoop(T &timerwheel) {
    int curfds = 1;
    int acceptCount = 0;
    ...
    for(int n = 0; n < _nfds; n++){
        if(_events[n].data.fd == _socket_fd){
            ...
            addFd(_conn_fd);
            timerwheel.addEvent(std::move(Timer(_conn_fd, )))
            curfds++;
            continue;
            ...
            continue;
        }else if(_events[n].data.fd == _timerFd){
            _timeoutFlag = true;
            char tempbuf[8];
            read(_timerFd, tempbuf, 8);
            continue;
